...
$awardVal = isset($_GET['awardVal']) ? explode(',', $_GET['awardVal']) : [];
$awardNames = json_decode(urldecode($_GET['awardNames']), true);
...
$positionDates = isset($_GET['dates']) ? json_decode($_GET['dates'], true) : [];
$positionTime = isset($_GET['position']) ? explode(',', $_GET['position']) : [];
?>
<script>
    // Get the user details and other variables from PHP
    var userDetails = JSON.parse(<?php echo json_encode($userDetails); ?>);
    ...
    var positionTime = <?php echo json_encode($positionTime); ?>;
    var positionDates = <?php echo json_encode($positionDates); ?>;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['userDetails'])) {
    $userDetails1 = json_decode($_POST['userDetails'], true);
    $year = $_POST['year'];
    $awardVal1 = json_decode($_POST['awardVal'], true);
    $researchVal1 = json_decode($_POST['researchVal'], true);
    $extensionVal1 = json_decode($_POST['extensionVal'], true);
    $awardNames1 = json_decode($_POST['awardNames'], true);
    $researchNamesa = json_decode($_POST['researchNames'], true);
    $researchNamesb = json_decode($_POST['researchNames1'], true);
    $extensionNames1 = json_decode($_POST['extensionNames'], true);
    $positionDates1 = json_decode($_POST['dates'], true);
    $positionTime1 = json_decode($_POST['time'], true);
    $tpr_score1 = json_decode($_POST['tpr_score'], true);
    $scoreArr1 = json_decode($_POST['scoreArr'], true);
    $selectedPercentage1 = json_decode($_POST['selectedPercentage'], true);

    // create a new PDO instance
    $pdo = new PDO('mysql:host=localhost;dbname=tpr', 'root', 'Siva@2000');
    // Check if the row already exists
    $eno = $userDetails1['eno'];
    $abs_year1 = $year;
    $year1 = $eno . '(' . $year . ')';
    $stmt = $pdo->prepare("SELECT COUNT(*) FROM tpr_awards1 WHERE year = :year");
    $stmt->bindParam(':year', $year1);
    $stmt->execute();
    $rowCount = $stmt->fetchColumn();

    if ($rowCount > 0) {
        // Row already exists, delete it first
        $deleteStmt = $pdo->prepare("DELETE FROM tpr_awards1 WHERE year = :year");
        $deleteStmt->bindParam(':year', $year1);
        $deleteStmt->execute();
    }
    // prepare the SQL query
    $stmt = $pdo->prepare("INSERT INTO tpr_awards1 (eno, year, opinion,feedback,abs_year) VALUES (:eno, :year, :opinion,:feedback,:abs_year)");
    $scoreArr = json_encode($scoreArr1, JSON_UNESCAPED_UNICODE);
    // bind parameters to the statement
    $stmt->bindParam(':eno', $eno1);
    $stmt->bindParam(':year', $year1);
    $opinionValue = json_encode($scoreArr1);
    $stmt->bindParam(':opinion', $opinionValue);
    $stmt->bindParam(':feedback', $selectedPercentage1);
    $stmt->bindParam(':abs_year', $abs_year1);

    $eno1 = $userDetails1['eno'];
    ...
    if ($stmt->errorInfo()[0] !== '00000') {
        echo "Error inserting data into tpr_awards1 table: " . $stmt->errorInfo()[2];
    }
    // Retrieve the values from the PHP array
    $SNo = $userDetails1['sno'];
    $ENo = $userDetails1['eno'];
    $name = $userDetails1['name'];
    $dept = $userDetails1['dept'];
    $gender = $userDetails1['gender'];
    $cat = $userDetails1['category'];
    $pos = $userDetails1['position'];
    $dob = $userDetails1['dob'];
    $doj = $userDetails1['doj'];
    $HQ = $userDetails1['high_qual'];
    $PAN = $userDetails1['pan_no'];
    $Aadhar = $userDetails1['aadhar_no'];
    $ph_no = $userDetails1['phone_no'];
    $email = $userDetails1['email_id'];
    ...
    if ($rowCount > 0) {
        // Row already exists, delete it first
        $deleteStmt = $pdo->prepare("DELETE FROM faculty_details WHERE year = :year");
        $deleteStmt->bindParam(':year', $year1);
        $deleteStmt->execute();
    } else {
        // Prepare the SQL query
        ...
        $errorInfo = $stmt->errorInfo();
        if ($stmt->errorInfo()[0] !== '00000') {
            echo "Error inserting data into faculty_details table: " . $stmt->errorInfo()[2];
        }
    }
    // Check if the row already exists
    ...
    if ($rowCount > 0) {
        // Row already exists, delete it first
        $deleteStmt = $pdo->prepare("DELETE FROM tpr_awards WHERE year = :year");
        $deleteStmt->bindParam(':year', $year1);
        $deleteStmt->execute();
    }
    ...
    $awards = json_encode($awardNames1[0], JSON_UNESCAPED_UNICODE);
    $teacherDistinction = json_encode($awardNames1[1], JSON_UNESCAPED_UNICODE);
    $books = json_encode($awardNames1[2], JSON_UNESCAPED_UNICODE);
    ...
    $stmt->bindParam(':leave_time', $awardVal1[2]);
    $stmt->bindParam(':books', $books);
    $stmt->bindParam(':tpr_score', $tprScore1);
    $stmt->bindParam(':abs_year', $abs_year1);
    ...
    $errorInfo = $stmt->errorInfo();
    if ($stmt->errorInfo()[0] !== '00000') {
        echo "Error inserting data into tpr_awards table: " . $stmt->errorInfo()[2];
    }
    ...
    if ($rowCount > 0) {
        // Row already exists, delete it first
        $deleteStmt = $pdo->prepare("DELETE FROM tpr_research1 WHERE year = :year");
        $deleteStmt->bindParam(':year', $year1);
        $deleteStmt->execute();
    }
    ...
    $journals = json_encode($researchNamesa[0], JSON_UNESCAPED_UNICODE);
    $sem_atd = json_encode($researchNamesa[1], JSON_UNESCAPED_UNICODE);
    $sem_org = json_encode($researchNamesa[2], JSON_UNESCAPED_UNICODE);
    $consultancy = json_encode($researchNamesa[3], JSON_UNESCAPED_UNICODE);
    // Bind parameters to the statement
    $stmt->bindParam(':eno', $eno);
    $stmt->bindParam(':year', $year1);
    $stmt->bindParam(':journals', $journals);
    $stmt->bindParam(':sem_atd', $sem_atd);
    $stmt->bindParam(':sem_org', $sem_org);
    $stmt->bindParam(':consultancy', $consultancy);
    $stmt->bindParam(':abs_year', $abs_year1);
    ...
    $errorInfo = $stmt->errorInfo();
    if ($stmt->errorInfo()[0] !== '00000') {
        echo "Error inserting data into tpr_det table: " . $stmt->errorInfo()[2];
    }

    // Repeat the above logic for the remaining sections:
    // tpr_research2
    ...
    if ($rowCount > 0) {
        // Row already exists, delete it first
        $deleteStmt = $pdo->prepare("DELETE FROM tpr_research2 WHERE year = :year");
        $deleteStmt->bindParam(':year', $year1);
        $deleteStmt->execute();
    }
    ...
    $guidance = json_encode($researchNamesb[0], JSON_UNESCAPED_UNICODE);
    $publications_total = json_encode($researchNamesb[2], JSON_UNESCAPED_UNICODE);
    $publications_3 = json_encode($researchNamesb[3], JSON_UNESCAPED_UNICODE);
    $patents = json_encode($researchNamesb[4], JSON_UNESCAPED_UNICODE);
    $projects_val = json_encode($researchNamesb[1]);
    // Bind parameters to the statement
    $stmt->bindParam(':eno', $eno);
    $stmt->bindParam(':year', $year1);
    $stmt->bindParam(':guidance', $guidance);
    $stmt->bindParam(':projects_val', $projects_val);
    ...
    if ($rowCount > 0) {
        // Row already exists, delete it first
        $deleteStmt = $pdo->prepare("DELETE FROM extension WHERE year = :year");
        $deleteStmt->bindParam(':year', $year1);
        $deleteStmt->execute();
    }
    ...
    $activities = json_encode($extensionNames1[0], JSON_UNESCAPED_UNICODE);
    $membership = json_encode($extensionNames1[1], JSON_UNESCAPED_UNICODE);
    $assignments = json_encode($extensionNames1[2], JSON_UNESCAPED_UNICODE);
    ...
    if ($rowCount > 0) {
        // Row already exists, delete it first
        $deleteStmt = $pdo->prepare("DELETE FROM pstn WHERE year = :year");
        $deleteStmt->bindParam(':year', $year1);
        $deleteStmt->execute();
    }
    ...
    $lecturer = json_encode($positionDates1[0], JSON_UNESCAPED_UNICODE);
    $reader = json_encode($positionDates1[1], JSON_UNESCAPED_UNICODE);
    $professor = json_encode($positionDates1[2], JSON_UNESCAPED_UNICODE);
...
<body>
    <div class="bdy">
            ...
            <table class="center page3" id="myTable2">
                <tbody>
                    ...
                    <tr>
                        <td>I</td>
                        <td>
                            <h1>Teaching and contribution to Academia:</h1>A) Opinion of the Head Department/Principal(Max. score : 120)

                            Assessment of the teacher on a 5-grade scale where grades 1 to 5 correspond to : poor, satisfactory, good, very good and excellent respectively
                            1. Availability to students during working hours
                            2. Punctuality to classes
                            3. Regularity in taking classes as per Time-Table
                            4. Maintenance of students attendance register
                            5. Maintenance of lecture diary
                            6. Arranging classroom seminars to students
                            7. Regularity in giving home assignments
                            8. Celerity/briskness in providing solutions to assignments
                            9. Promptness in returning answer scripts to students
                            10. Availability for examination work including invigilation
                            11. Participation in curriculum / syllabus development
                            12. Participation in college/ Department activities
                        </td>
                        <td>120</td>
                        <td></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td>B) Student feedback on teacher performance (last 3 years)(Max. score : 120)

                            Assessment of the teacher on a 5-grade scale where grades 1 to 5 correspond to increasing teacher quality

                            % of students who assessed the
                            teacher with grade-4 and/or grade-5     Score
                            above 95                                                    120
                            90-&lt;95                                                       110
                            85-&lt;90                                                       100
                            80-&lt;85                                                        90
                    ...
                    <tr>
                        <td>II </td>
                        <td>
                            <h1>Research and Consultancy</h1>A) Research Guidance (Max. Score : 100)
                            i) M.Phil / M.Tech / M.Pharm/LL.M : 5
                            (each)
                            ii) Ph.D : 10 (each)
                            iii) Students presently working for Ph.D : 2 (each)
                        </td>
                        <td>100</td>
                        <td></td>
                    </tr>
                    <tr>
                        ...
                        <td>B) Research Projects operated / under operation
                            ...
                            Score
                            Rs. 80 lakh or more : 25
                            Rs. 60 -&lt;80 lakh : 20
                            Rs. 40 -&lt;60 lakh : 15
                            Rs. 20 -&lt;40 lakh : 10
                            < Rs. 20 lakhs=: 5 if arts->
                                Rs. 40 lakh or more : 25
                                Rs. 30 -&lt;40 lakh : 20
                                Rs. 20 -&lt;30 lakh : 15
                                Rs. 10 -&lt;20 lakh : 10
                                < Rs. 10 lakhs=: 5 3) Number of Projects with funding from outside India (Max. Score: 10) Each : 5 4) Number of projects having collaboration with industry / other research organizations (Max. Score: 10) Each : 5 5) Number of projects having collaboration with other departments in the University (Max. Score: 10) Each : 5 </td>
                        <td>100</td>
                        <td></td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        <td>III </td>
                        <td>
                            <h1>Extension / Other Activities</h1>A) Extension / out reach activities involved in
                            (Max. Score : 100)
                            ...
                            For any recognized and authenticated activity : 20 (each)
                        </td>
                        <td>100</td>
                        <td></td>
                    </tr>
        ...
        <div class="footer">
            ...
            <br><br><br>
            <h2 class="sig">
                <p id="signature"></p><i>SIGNATURE</i>
            </h2>
        </div>
    </div>
    <form method="POST" action="">
        ...
        <input type="hidden" name="scoreArr">
        <input type="hidden" name="selectedPercentage">
    ...
    <script>
        ...
        document.querySelector('input[name="scoreArr"]').value = JSON.stringify(scoreArr);
        document.querySelector('input[name="selectedPercentage"]').value = JSON.stringify(selectedPercentage);
    </script>
    <script>
        // Parse the date string into a Date object
        var parts = userDetails.doj.split('/');
        ...
        // Adjust the age if the current month and day are before the birthday
        ...
        row = table1.rows[1];
        cell = row.cells[2];
        cell.innerHTML = userDetails.position;
        row = table1.rows[2];
        cell = row.cells[2];
        cell.innerHTML = userDetails.dept;
        row = table1.rows[3];
        cell = row.cells[2];
        cell.innerHTML = userDetails.phone_no;
        row = table1.rows[4];
        cell = row.cells[2];
        cell.innerHTML = userDetails.email_id;
        row = table1.rows[5];
        cell = row.cells[2];
        ...
        row = table1.rows[7];
        cell = row.cells[2];
        cell.innerHTML = userDetails.high_qual;
        row = table1.rows[8];
        cell = row.cells[2];
        cell.innerHTML = age;
        ...
        row = table2.rows[1];
        cell = row.cells[3];
        cell.innerHTML = scoreArr;
        row = table2.rows[2];
        cell = row.cells[3];
        cell.innerHTML = parseInt(selectedPercentage);
        row = table2.rows[3];
        cell = row.cells[3];
        ...
        sum += parseInt(scoreArr) + parseInt(selectedPercentage);
        row = table2.rows[18];
        cell = row.cells[3];
        cell.innerHTML = sum;
        ...
        cell = row.cells[2];
        cell.innerHTML = sum;
        var tprScoreInput = document.querySelector("input[name='tpr_score']");
        tprScoreInput.value = sum;
